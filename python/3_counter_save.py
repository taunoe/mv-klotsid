...
MODEL_FILE = '../models/klotsid_2_v8.pt'
CONF_TH = 0.6
...
# Video file
cap = cv2.VideoCapture('../images/test_video_5.mp4')
out_file_name = '../output/test_video_2_eng.avi'
WIDTH = 1920
HEIGHT = 1080

line_pt = [int((WIDTH/2)), 50, int((WIDTH/2)), (HEIGHT-50)] # Loendus joon x1

mask = cv2.imread('mask_1920_1080.png')  # sektor, kus loeme


# Save video file
result_video = cv2.VideoWriter(out_file_name ,
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if conf > CONF_TH:
                ...
                # Write Name:
                cvzone.putTextRect(frame,f'{current_name}',(max(0,x1), max(30, y1)),scale=1.8,thickness=2,offset=5,colorR=colors[name_id],colorT=(10,10,10))
    ...
    for result in results_tracker:
        ...
        if line_pt[1] < cy < line_pt[3] and line_pt[0]-20 < cx < line_pt[2]+20:
            if total_counter.count(id) == 0: # kui ei ole juba loetud
                ...
                padding = 30
                cv2.rectangle(count_mask, ((x1-padding), (y1-padding)), ((x2+padding), (y2+padding)), 255, -1)
                ...
                cboxes = count_results[0].boxes # Ainult esimene
                
                for cbox in cboxes:
                    _id = int(cbox.cls[0]) # class_id
